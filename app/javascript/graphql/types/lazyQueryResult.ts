...
export type RefetchLazyQueryFunction<Model, Variables, ResultKey extends string> = 
  (variables?: Partial<OperationVariables | Variables>) => Promise<
    ApolloQueryResult<
      OperationVariables | Variables | Model | { [Key in ResultKey]?: Model; }
    >
  >;

export type LazyQueryExecuteFunction<Model, Variables, ResultKey extends string> =
...
interface LazyQueryResult<Model, Variables, ResultKey extends string> {
  data?: Variables | Model | OperationVariables | { [Key in ResultKey]?: Model; };
  loading: boolean;
  called: boolean;
